...
class Estados extends CI_Controller {
    ...
    public function index() {
        ...
        if (!empty($Buscar) || isset($estatus))
            $total_rows = count($this->EstadosModel->filtros($estatus, $Buscar));
        else
            $total_rows = $this->EstadosModel->record_count();

        $config['base_url'] = base_url('estados/index');
        ...
        if ($perPage != null) {
            ...
            $datos["resultado"] = $this->EstadosModel->fetch_data($total_rows, $page, $estatus, $Buscar, $config['per_page']);
        } else {
            $page = 0;
            $datos["resultado"] = $this->EstadosModel->fetch_data($config['per_page'], $page, $estatus, $Buscar, $config['per_page']);
        }
        ...
        $this->load->view('listEstados', $datos);
    ...
    public function guardar() {
        ...
        $respuesta = $this->EstadosModel->agregar($dato);
    ...
    public function editar() {
        ...
        $data['usuario'] = $this->session->usuario;
        $this->load->view('Templates/header', $data);
        $this->load->view('frmEditarEstado');
        $this->load->view('Templates/footer');
    }
    ...
    public function actualizar() {
        ...
        $respuesta = $this->EstadosModel->actualizar($busqueda);
        echo json_encode(array('data' => $respuesta));
    }

    public function buscar() {
        $respuesta = $this->EstadosModel->getAll();
        echo json_encode(array('data' => $respuesta));
    }
    ...
    public function buscarPorId() {
        $postdata = file_get_contents('php://input');
        $busqueda = json_decode($postdata);
        $respuesta = $this->EstadosModel->buscarId($busqueda->id);
