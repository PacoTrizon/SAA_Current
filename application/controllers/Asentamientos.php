...
class Asentamientos extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->getSessionUser();
        $this->load->model('AsentamientosModel');
    ...
    public function index() {
        ...
        if (!empty($Buscar) || isset($estatus))
            $total_rows = count($this->AsentamientosModel->filtros($estatus, $Buscar));
        else
            $total_rows = $this->AsentamientosModel->record_count();

        $config['base_url'] = base_url('asentamientos/index');
        ...
        if ($perPage != null) {
            ...
            $datos["resultado"] = $this->AsentamientosModel->fetch_data($total_rows, $page, $estatus, $Buscar, $config['per_page']);
        } else {
            $page = 0;
            $datos["resultado"] = $this->AsentamientosModel->fetch_data($config['per_page'], $page, $estatus, $Buscar, $config['per_page']);
        }
        ...
        $this->load->view('listAsentamientos', $datos);
    ...
    public function agregar() {
        ...
        $data['usuario'] = $this->session->usuario;
        $this->load->view('Templates/header', $data);
        $this->load->view('frmAsentamiento');
        $this->load->view('Templates/footer');
    }
    ...
    public function guardar() {
        ...
        $respuesta = $this->AsentamientosModel->agregar($datos);
    ...
    public function editar() {
        ...
        $data['usuario'] = $this->session->usuario;
        $this->load->view('Templates/header', $data);
        $this->load->view('frmEditarAsentamiento');
        $this->load->view('Templates/footer');
    }
    ...
    public function actualizar() {
        $postdata = file_get_contents('php://input');
        $datos = json_decode($postdata);
        $respuesta = $this->AsentamientosModel->actualizar($datos);
        echo json_encode(array('data' => $respuesta));
    }
    ...
    public function buscar() {
        $postdata = file_get_contents('php://input');
        $datos = json_decode($postdata);
        $respuesta = $this->AsentamientosModel->getAll();
        echo json_encode(array('data' => $respuesta));
    }
    ...
    public function buscarPorId() {
        $postdata = file_get_contents('php://input');
        $datos = json_decode($postdata);
        $respuesta = $this->AsentamientosModel->buscarId($datos->id);
